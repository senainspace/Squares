...
namespace PBL_Squares
{
    internal class Program
    {
        ...
        static void Main()
        {
            ...
            CursorMode(square, numberOfPieces);
        ...
        static void CursorMode(int[] square, int numberOfPieces)
        {
            ConsoleKey key;

            while (true)
            {
                DrawPuzzleWithCursor(square, numberOfPieces);
                ...
                switch (key)
                {
                    ...
                    case ConsoleKey.DownArrow:
                        if (cursorRow < puzzle.GetLength(0) - 1) cursorRow++;
                        break;
                    ...
                    case ConsoleKey.RightArrow:
                        if (cursorCol < puzzle.GetLength(1) - 1) cursorCol++;
                        break;

                    case ConsoleKey.Spacebar:
                    case ConsoleKey.Enter:
                        puzzle[cursorRow, cursorCol] = currentLetter;
                        break;
        ...
        static void DrawPuzzleWithCursor(int[] square, int numberOfPieces)
        {
            Console.Clear();
            PrintGameScreen(square, numberOfPieces);
            
            int puzzleStartRow = 2; 
            int puzzleStartCol = 2;

            Console.SetCursorPosition(puzzleStartCol + cursorCol, puzzleStartRow + cursorRow);

            Console.BackgroundColor = ConsoleColor.White;
            Console.ForegroundColor = ConsoleColor.Black;

            Console.Write(puzzle[cursorRow, cursorCol]);

            Console.ResetColor();

            Console.SetCursorPosition(0, puzzleStartRow + puzzle.GetLength(0) + 2);
            Console.WriteLine($"Cursor: ({cursorRow + 1},{cursorCol + 1})  Current Letter: {currentLetter}");
            Console.WriteLine("Arrow Keys: Move | A-Z: Select Letter | ENTER/SPACE: Place | ESC: Exit");
        }
